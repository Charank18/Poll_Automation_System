  ...
  errorComponent: ({ error }) => {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center p-8 bg-red-50 rounded-lg shadow-lg max-w-md">
          <h1 className="text-2xl font-bold text-red-800 mb-4">Something went wrong</h1>
          <p className="text-red-600 mb-6">{error instanceof Error ? error.message : 'Unexpected error'}</p>
          <button 
            className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700"
            onClick={() => window.location.href = '/auth'}
          >
...
const authRoute = new Route({
  ...
  beforeLoad: () => {
    const { isAuthenticated, user } = useAuthStore.getState();
    if (isAuthenticated && user?.role) {
      throw redirect({ to: `/${user.role.toLowerCase()}` });
    }
  },
});
...
export const router = new Router({
  routeTree,
  defaultPreload: 'intent',
  history: typeof window !== 'undefined' ? undefined : createMemoryHistory(),
  defaultNotFoundComponent: NotFoundComponent,
  notFoundRoute,
});
